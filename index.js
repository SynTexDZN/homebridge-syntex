...
class SynTexPlatform
{
	...
	initWebServer()
	{
		...
		this.WebServer.addPage(['/debug/workaround/', '/debug/workaround/index'], async (response, urlParams, content) => {
			...
			var storageAccessories = DeviceManager.getAccessories();

			if(bridgeAccessories != null)
			{
				for(const i in bridgeAccessories)
				{
					for(const j in storageAccessories)
					{
						if(storageAccessories[j].id == bridgeAccessories[i].id)
						{
							for(const x in storageAccessories[j])
							{
								if(x != 'id'/* && x != 'version' && x != 'name' && x != 'plugin' && x != 'services'*/)
								{
									bridgeAccessories[i][x] = storageAccessories[j][x];
								}
							}
						}
					}
				...
				obj.devices = JSON.stringify(bridgeAccessories);
			}
			else if(storageAccessories != null)
			{
				obj.devices = JSON.stringify(storageAccessories);
			}
	...
	getBridgeAccessories()
	{
		return new Promise((resolve) => {
			...
			var theRequest = {
				method : 'GET',
				url : 'http://localhost:' + (DeviceManager.getBridgePort() || '51826') + '/accessories',
				timeout : 10000
			};
