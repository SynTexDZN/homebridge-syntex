...
module.exports = class StateManager
{
	constructor(platform)
	{
        ...
        this.dataSystem = new AutomationSystem(platform);
        ...
        this.loadData().then(() => {
            ...
            setInterval(() => {
                ...
                if(this.changed)
                {
                    this.files.writeFile('cache/accessories.json', this.data).then((response) => {
    
                        if(response.success)
                        {
                            this.logger.log('update', 'bridge', 'Bridge', 'Accessories wurden aktualisiert!');
                        }
                        else
                        {
                            this.logger.log('error', 'bridge', 'Bridge', 'Accessories konnten nicht aktualisiert werden!');
                        }
                    });
